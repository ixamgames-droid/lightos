...
- **Quellenwechsel von außen** (BPM-14): `SourceController.subscribe_change(cb)` →
  ...
  werden einfach nicht abonniert. Von außen schalten der Generator-Knopf (`apply("song")`)
  und die VC-Taste „Musik-BPM" (`toggle_audio()`, BPM-16).
...
- `tests/test_bpm16_musik_bpm_controller.py` — VC-Taste „Musik-BPM" an/aus: Liste und
  `ctrl.kind` folgen, Capture/OS2L passend, je Druck ein Schaltvorgang; zuletzt gewählte
  Quelle bzw. gespeicherte Einstellung; Rückweg nach „BPM = 0/aus"; APC-LED.
