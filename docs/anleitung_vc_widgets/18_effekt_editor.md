...
> Eine bewegliche Box, die einen Effekt live im Miniaturformat vorschaut und darunter automatisch die passenden Bedien-Fader (Speed, Intensität, Size) bereitstellt — der „Streifen“ für einen einzelnen Effekt.
...
- **darunter:** automatisch erzeugte **Fader** für die sinnvollen Numerik-Parameter des Effekts (Speed/Tempo, Intensität, Size). Diese Fader wirken sofort auf den laufenden Effekt.
...
3. **Fader-Bereich (unten):** bis zu **drei** schmale Fader (im Bild zwei, jeweils mit Prozent-Anzeige „62 %“ obendrauf). Jeder Fader steuert einen Live-Parameter des Effekts.
...
> Hinweis: Die Box hat **keinen eigenen Toolbar-Knopf**. Sie entsteht über **Smart-Drop** — du ziehst einen Effekt aus der Bibliothek auf die Canvas, und LightOS legt für passende Effekte automatisch diese Box samt Vorschau und Fadern an.
...
- **Binden:** Du bindest nicht von Hand — die Bindung entsteht automatisch beim **Smart-Drop** (Effekt aus der Bibliothek auf die Canvas ziehen). LightOS prüft den Zieleffekt: Nur Effekte mit einem **Algorithmus und Live-Parametern** (Matrix/EFX) bekommen die Box samt Vorschau und Fadern.
- **Was beim Binden passiert:** Der Header übernimmt den Effektnamen; die Vorschau wird auf die echte Geometrie des Effekts (Spalten/Reihen, Farben, Geschwindigkeit) gesetzt; pro sinnvollem Parameter aus `Speed/Tempo`, `Intensität`, `Size` wird **ein Fader** unter die Vorschau gehängt (maximal drei).
- **Ohne brauchbare Bindung:** Für Effekte ohne Live-Parameter (z. B. Szene/Chaser) wird **kein** Vorschau-Band und **kein** Fader gebaut — die Box bleibt leer bzw. wird gar nicht erst als Editor angelegt.
- **Live-Wirkung:** Die Fader wirken über die gemeinsame Effekt-Naht (`src/core/engine/effect_live.py`): Ein Fader-Wert (0–100 %) wird linear auf den echten Wertebereich des Parameters abgebildet und sofort gesetzt. Es wird nur die Effekt-ID gespeichert — die Wirkung läuft immer über den aktuell gebundenen Effekt.
...
- **Maximal drei Fader:** Es werden nur die ersten drei passenden Numerik-Parameter (Speed/Intensität/Size) als Fader angelegt; Speed und Matrix-Speed gelten dabei als ein Parameter. Mehr Live-Parameter erreichst du über das Kontextmenü der Fader.
