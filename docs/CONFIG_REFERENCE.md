...
| `LIGHTOS_NO_RECOVERY_PROMPT` | Unterdrückt den modalen Autosave-Recovery-Dialog beim MainWindow-Bau (headless würde ihn niemand beantworten). **Wirkt NUR dort** — die „Show speichern?"-Abfrage beim Beenden hat seit 2026-07-26 einen eigenen Guard (`_exit_prompt_suppressed`, nur `QT_QPA_PLATFORM=offscreen`), damit dieser Schalter keinen stillen Datenverlust auslösen kann. | nicht gesetzt | `src/ui/main_window.py:140` | `tests/conftest.py:99` setzt es; Regressionstest `tests/test_autosave_recovery_headless.py`. Zweites Netz: `QT_QPA_PLATFORM=offscreen`. |
...
| `LIGHTOS_FIXTURE_DB` | Pfad der Fixture-Library-Datenbank (SQLite). | `app_data_dir()/fixtures.db` | `src/core/database/fixture_db.py:15` | `tests/conftest.py` legt pro Prozess eine **Kopie** der realen Bibliothek an und zeigt darauf: kopiert wegen der festen `fixture_profile_id` der committeten Shows, Kopie statt Original, weil `get_engine()` das Schema migriert (QA-58). Wächter: `tests/conftest.py` prüft in **jedem** Segment, ob `fixture_db` doch auf `app_data_dir()/fixtures.db` zeigt (`pytest_collection_finish` für `DB_PATH`, Fixture `_waechter_bibliothek_ist_eine_kopie` für die gebaute Engine); belegt in `tests/test_qa58_bibliothek_schema_unberuehrt.py`. |
